...
static void show_ota_page(void)
{
    ...
    label(scr, "No upgrade package found", 132, 415, &lv_font_montserrat_16, c_muted());
    label(scr, "Entering dashboard...", 154, 446, &lv_font_montserrat_14, c_ai());
    load_screen(scr);
}
...
void EnergyLvgl_Task(void const *argument)
{
    ...
    EnergyLvgl_PortInit();
    show_ota_page();
    lv_timer_create(ota_done_cb, 1400, NULL);
