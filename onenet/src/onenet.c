...
#define PROID			"5KnNRqC06w"

#define ACCESS_KEY		"RVFCcllsRTU3VDNDOEJjZ3B1Rm5zMW9TQlpGaWxRdmY="

#define DEVICE_NAME		"wlh"
...
extern unsigned char esp8266_buf[512];//512
											...
											char *authorization_buf, unsigned short authorization_buf_len, _Bool flag)
{
	...
	char sign_buf[64];//64								//保存签名的Base64编码结果 和 URL编码结果
	char hmac_sha1_buf[64];//64							//保存签名
...
_Bool OneNet_DevLink(void)
{
	...
	char authorization_buf[160];//160
	...
	if(MQTT_PacketConnect(PROID, authorization_buf, DEVICE_NAME, 1000, 1, MQTT_QOS_LEVEL0, NULL, NULL, 0, &mqttPacket) == 0)
	{
		...
		dataPtr = ESP8266_GetIPD(250);	//250								//等待平台响应
		if(dataPtr != NULL)
		{
...
extern uint8_t smog_th,co_th,pm_th,air_th;
extern uint8_t Beep_Status;
extern uint16_t lux_th;
...
//	
//	memset(text, 0, sizeof(text));
//	sprintf(text, "\"air\":{\"value\":%d},", air_value);
//	strcat(buf, text);
//	
//	
//	memset(text, 0, sizeof(text));
//	sprintf(text, "\"lux\":{\"value\":%d},", lux);
//	strcat(buf, text);
//	
//	memset(text, 0, sizeof(text));
...
//	
//	memset(text, 0, sizeof(text));
//	sprintf(text, "\"smog_th\":{\"value\":%d},", smog_th);
//	strcat(buf, text);
//	
//	
//	memset(text, 0, sizeof(text));
//	sprintf(text, "\"lux_th\":{\"value\":%d},", lux_th);
//	strcat(buf, text);
//	
//	memset(text, 0, sizeof(text));
//	sprintf(text, "\"air_th\":{\"value\":%d},", air_th);
//	strcat(buf, text);
...
//	UsartPrintf(USART_DEBUG, "Tips:	OneNet_SendData-MQTT\r\n");
	...
	UsartPrintf(USART_DEBUG, "-------%s--------",buf);//看是否发送上去
	UsartPrintf(USART_DEBUG, "-------%d--------",body_len);//看长度是否够
	if(body_len)
	{
...
void OneNet_RevPro(unsigned char *cmd)
{
	...
	cJSON *raw_json, *params_json, *fan_json, *smog_th_json, *led_json,*beep_json,*lux_th_json,*air_th_json;
	...
	switch(type)
	{
		case MQTT_PKT_PUBLISH:																//接收的Publish消息
			...
			if(result == 0)
			{
				...
				if(fan_json != NULL)
				{
					if(fan_json->type == cJSON_True) Fan_Set(1);
					else Fan_Set(0);
				}
				...
				lux_th_json = cJSON_GetObjectItem(params_json,"lux_th");
				if(lux_th_json != NULL)
				{
					lux_th = lux_th_json->valuedouble;
				}
				air_th_json = cJSON_GetObjectItem(params_json,"air_th");
				if(air_th_json != NULL)
				{
					air_th = air_th_json->valuedouble;
				}
